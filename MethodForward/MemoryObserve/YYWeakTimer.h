...
//  YYMobileFramework
//
//  Created by Arcfat Tsui on 12/30/16.
//  Copyright © 2016 YY Inc. All rights reserved.
//

#import <Foundation/Foundation.h>

FOUNDATION_EXPORT double YYWeakTimeVersionNumber;
FOUNDATION_EXPORT const unsigned char YYWeakTimerVersionString[];

/**
 `YYWeakTimer` behaves similar to an `NSTimer` but doesn't retain the target.
 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `YYWeakTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `YYWeakTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `YYWeakTimer` won't invoke the `selector` on `target` again after calling this method.
